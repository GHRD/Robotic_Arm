...
class RoboticArm:
    ...
    # Homogeneous transformation matrices for each joint
        T_01 = self.DH_matrix(joint_angles[0], self.link_lengths[0], self.link_offsets[0], -np.pi/2)
        T_12 = self.DH_matrix(joint_angles[1], self.link_lengths[1], self.link_offsets[1], 0)
        T_23 = self.DH_matrix(joint_angles[2], self.link_lengths[2], self.link_offsets[2], -np.pi/2)
    ...
    # Calculate the Jacobian matrix
        J = np.zeros((3, 3))
        J[:, 0] = np.cross([0, 0, 1], pos - T_01[:3, 3])
        J[:, 1] = np.cross(T_01[:3, 2], pos - T_12[:3, 3])
        J[:, 2] = T_01[:3, 2]
